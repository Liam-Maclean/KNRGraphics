...
Texture2D Tex2DArray[] : register(t7, space1);
...
PS_OUTPUT PackGBuffer(float3 baseColor, float3 normalVS, float4 emissive, float roughness, float metallic, float ao)
{
	...
	gBufferOut.emissive = float4(emissive.xyz, ao);
	return gBufferOut;
}
...
PS_OUTPUT main(VS_OUTPUT input) : SV_TARGET
{
	...
	if (materialBuffer.albedo_idx > -1)
	{
		...
		diffuseColor = txAlbedo.Sample(anisotropicSampler, input.texCoord) * materialBuffer.baseColorFactor;
	...
	//Pack the G buffer
	PS_OUTPUT gBufferOut = PackGBuffer(diffuseColor.xyz, normal, emissiveColor, roughness, metallic, occlusionColor.r);
