#include <pix_win.h>
#include "opengl_graphics_profiler.h"
...
namespace KNR
{
	...
	{
		return new OpenglGraphicsProfiler();
	}
	...
	OpenglGraphicsProfiler::OpenglGraphicsProfiler()
	{
	...
	uint64_t OpenglGraphicsProfiler::StartProfiler(CommandBuffer* commandList, const char* name)
	{
		uint64_t profileIdx;
		return profileIdx;
	}

	void OpenglGraphicsProfiler::EndProfiler(CommandBuffer* commandList, uint64_t idx)
	{
	...
	void OpenglGraphicsProfiler::DisplayFrameData()
	{
	...
	void OpenglGraphicsProfiler::Update(ProfileData& profile, uint64_t profileIdx, bool drawText, uint64_t gpuFrequency, uint64_t* frameQueryData)
	{
		m_frameCount++;
	...
	void OpenglGraphicsProfiler::CreateReadbackBuffer()
	{
