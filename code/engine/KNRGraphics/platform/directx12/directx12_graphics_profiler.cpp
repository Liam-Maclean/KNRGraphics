...
namespace KNR
{
	...
	uint64_t DirectX12GraphicsProfiler::StartProfiler(CommandBuffer* commandList, const char* name)
	{
		...
		PIXBeginEvent(directXCommandList->Get(), 0, name);
		...
		directXCommandList->Get()->EndQuery(m_queryHeap, D3D12_QUERY_TYPE_TIMESTAMP, startQueryIdx);
	...
	void DirectX12GraphicsProfiler::EndProfiler(CommandBuffer* commandList, uint64_t idx)
	{
		...
		directXCommandList->Get()->EndQuery(m_queryHeap, D3D12_QUERY_TYPE_TIMESTAMP, endQueryIdx);

		// Resolve the data
		const uint64_t dstOffset = (startQueryIdx * sizeof(uint64_t));
		directXCommandList->Get()->ResolveQueryData(m_queryHeap, D3D12_QUERY_TYPE_TIMESTAMP, startQueryIdx, 2, m_readbackBuffer, dstOffset);
		PIXEndEvent(directXCommandList->Get());
	}

	void DirectX12GraphicsProfiler::StartPixMarker(CommandBuffer* commandList, const char* marker)
	{
	}

	void DirectX12GraphicsProfiler::EndPixMarker(CommandBuffer* commandList)
	{
	}
