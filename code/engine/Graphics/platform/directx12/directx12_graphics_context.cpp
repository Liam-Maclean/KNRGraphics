...
namespace KNR
{
	...
	void CDirectX12Context::Init(WindowDesc* windowHandle)
	{
		...
		m_blitFence = new DirectX12Fence(false);
	...
	void CDirectX12Context::EndBlit()
	{
	...
	void CDirectX12Context::CreateSwapchain()
	{
		//Liam fix - need IWindow size here (width and height)
		int width = m_window->width;
		int height = m_window->height;
		m_swapChain = new DirectX12Swapchain(m_window->hwnd, m_window->width, m_window->height);
	}
